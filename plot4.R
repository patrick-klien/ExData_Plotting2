## This script will result in a plot to answer question 4 for project 2 of the Exploratory Data Analysis course of Coursera.
...
plot4 <- function(){
  ...
  ## Subset the data so that we only have Coal emission data.
  data_final <- mergeData[grep("Coal", mergeData$EI.Sector),]
  ...
  plot(year_sum$Group.1, year_sum$x, type = "b", ylab = expression(PM[2.5]), xlab = "Years", xaxp = c(1999,2008,3), lwd = 4, main = expression("Total Coal emissions: PM"[2.5]))
  plot(year_avg$Group.1, year_avg$x, type = "b", ylab = expression(PM[2.5]), xlab = "Years", xaxp = c(1999,2008,3), lwd = 4, main = expression("Average Coal emissions: PM"[2.5]))
